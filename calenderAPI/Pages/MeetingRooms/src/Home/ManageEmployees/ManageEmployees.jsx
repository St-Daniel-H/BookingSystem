...
export default function ManageEmployees(props) {
    ...
    async function getAllEmployeesWithCompanyId() {
        try {
            const getEmployees = await fetch(APIs.apiLink + "/auth/company/" + companyId).then(
                (response) => {
                    if (response) {
  ...
  async function submitRoom(e) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorResponse = await response.json();
          console.log("Adding Employee failed:", errorResponse.$values[0].errorMessage);
    ...
    const [employeeToUpdate, setEmployeeToUpdate] = useState({ userId: "", firstName: "", lastName: 0, email:"",role:""})
    async function updateTheEmployeeOnClick(Id, firstName, lastName, role) {
        document.body.style.overflow = "hidden";
        setEmployeeToUpdate({ Id: Id, firstName: firstName, lastName: lastName, role: role });
    ...
    function search() {
        ...
        const array = employees.filter((x) => x.firstName.toLowerCase().includes(searchInput.toLowerCase()) || x.lastName.toLowerCase().includes(searchInput.toLowerCase()) );
   ...
   return (
    <div id="ManageRooms">
                  ...
                  <TextField
                      ...
                      />
                           ...
                           <Select
                               ...
                               <MenuItem value="Employee" default>Employee</MenuItem>
                           </Select>
              </div>
              <div id="rightSide">
                  <TextField
          ...
          <table>
              ...
              <tbody>
                  {!searching ?
                      employees.map((units, index) => {
                          return (
                              <tr key={index}>
                                  <td>{units.firstName} {units.lastName}</td>
                                  <td>{units.email}</td>
                                  <td>{units.role}</td>
                                  {isAdmin() ? <>
                                      <td><button onClick={() => {
                                          //document.body.style.overflow = "hidden";
                                          updateTheEmployeeOnClick(units.id, units.firstName, units.lastName, units.role, units.email, units.password);
                                      }}>Update</button></td>
                                      <td><button onClick={() => {
                               ...
                               return (
                                   <tr key={index}>
                                       ...
                                       {isAdmin() ? <>
                                           <td><button onClick={() => {
                                               //document.body.style.overflow = "hidden";
                                               updateTheEmployeeOnClick(units.id, units.firstName, units.lastName, units.role);
                                           }}>Update</button></td>
                                           <td><button onClick={() => {
                                               deleteTheEmployeeOnClick(units.Id)
                                           }}>Delete</button></td>
           ...
           {deleteTheEmployee ? <DeleteEmployee userToDelete={employeeToDelete} setState={setDeleteTheEmployee} state={deleteTheEmployee} /> : ""}
           {updateTheEmployee ? <UpdateEmployee companyId={companyId} userToUpdate={employeeToUpdate} setState={setUpdateEmployee} state={updateTheEmployee} /> : ""}
    </div>
  );
}
